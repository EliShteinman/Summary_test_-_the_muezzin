...
class MongoDBAsyncRepository:
    ...
    async def create(self, document: Dict[str, Any]) -> str:
        ...
        result = await self.client.execute_insert_one(
            self.collection_name, doc_with_meta
        )
        logger.info(f"Document created in {self.collection_name}: {result.inserted_id}")
        return str(result.inserted_id)

    async def get_by_id(
        self, doc_id: str, id_field: str = "_id"
    ...
    async def get_all(
        self, limit: int = 0, sort_by: List[tuple] = None, **filters
    ) -> List[Dict[str, Any]]:
        """Get all documents with optional filters (async)."""
        query = self._build_query(**filters)
        sort_by = sort_by or [("created_at", -1)]

        docs = await self.client.execute_find(
            self.collection_name, query, limit, sort_by
        )
        for doc in docs:
            doc["_id"] = str(doc["_id"])
        return docs
    ...
    async def update(
        self, doc_id: str, updates: Dict[str, Any], id_field: str = "_id"
    ) -> bool:
        ...
        result = await self.client.execute_update_one(
            self.collection_name, query, updates_with_meta
        )
    ...
    async def search(
        self, limit: int = 10, skip: int = 0, sort_by: List[tuple] = None, **filters
    ) -> Dict[str, Any]:
        """
        ...
        all_docs = await self.client.execute_find(
            self.collection_name, query, limit + skip, sort_by
        )
        ...
        # Get total count
        total_count = await self.client.execute_count_documents(
    ...
    async def bulk_create(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            result = await self.client.execute_insert_many(
                self.collection_name, docs_with_meta
            )
            return {
    ...
    async def bulk_delete(self, filters: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for filter_query in filters:
            try:
                result = await self.client.execute_delete_one(
    ...
    async def to_dataframe(self, **filters) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Created DataFrame with {len(df)} rows from {self.collection_name}"
            )
            return df
    ...
    async def bulk_create_from_dataframe(
        self, df: pd.DataFrame, id_column: str = None
    ) -> Dict[str, Any]:
        """
